...
function MenuRecognitionModal({restaurant,categories,itemCount,onClose,onSaved}:{restaurant:Restaurant;categories:Category[];itemCount:number;onClose:()=>void;onSaved:()=>void}){
  ...
  async function recognize(){if(files.length===0)return;setBusy(true);setError("");setResults([]);setRawText("");setProgress(1);setStatus("Texterkennung wird vorbereitet …");let worker:Awaited<ReturnType<typeof import("tesseract.js")["createWorker"]>>|null=null;const selected=[...files];try{const{createWorker}=await import("tesseract.js");const assetBase=process.env.NEXT_PUBLIC_BASE_PATH||"";let fileIndex=0;worker=await createWorker("deu",1,{workerPath:`${assetBase}/tesseract/worker.min.js`,corePath:`${assetBase}/tesseract/core`,langPath:`${assetBase}/tesseract/lang`,logger:message=>{if(message.status==="recognizing text")setProgress(Math.max(3,Math.round(((fileIndex+message.progress)/selected.length)*100)))}});const recognized:RecognizedMenuItem[]=[];const textParts:string[]=[];for(fileIndex=0;fileIndex<selected.length;fileIndex+=1){setStatus(`Bild ${fileIndex+1} von ${selected.length} wird analysiert …`);const{data}=await worker.recognize(selected[fileIndex]);textParts.push(data.text);recognized.push(...parseRecognizedMenu(data.text))}setRawText(textParts.join("\n\n--- NÄCHSTES BILD ---\n\n"));setResults(recognized);setProgress(100);setStatus(recognized.length?`${recognized.length} Einträge erkannt. Bitte kurz prüfen.`:"Analyse abgeschlossen.");if(recognized.length===0)setError("Es wurden keine Einträge mit Preis erkannt. Nutze ein scharfes, gerade aufgenommenes Bild oder prüfe unten den erkannten Text.")}catch(recognitionError){setError(recognitionError instanceof Error?recognitionError.message:"Die Speisekarte konnte nicht erkannt werden.");setStatus("")}finally{if(worker)await worker.terminate();setFiles([]);setInputVersion(version=>version+1);setBusy(false)}}
